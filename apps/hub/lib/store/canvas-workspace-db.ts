...
function rowToBoard(row: typeof schema.canvasBoards.$inferSelect): CanvasBoard {
  const document =
    (row.document as unknown as CanvasDocument) ?? emptyCanvasDocument();
  return {
    id: row.id,
...
export async function createCanvasBoard(
  ...
  return getCanvasBoard(db, userId, workspaceId, id);
}
...
export async function updateCanvasBoard(
  ...
  if (!(await assertMember(userId, workspaceId))) return null;
  const current = await getCanvasBoard(db, userId, workspaceId, boardId);
  ...
  const board = await getCanvasBoard(db, userId, workspaceId, boardId);
...
export async function deleteCanvasBoard(
  ...
  await db.delete(schema.canvasBoards).where(eq(schema.canvasBoards.id, boardId));
  return true;
}
